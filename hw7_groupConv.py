...
class Classifier(nn.Module):
    ...
    def __init__(self, base=16, width_mult=1):
        ...
        groups_num = [ 8  for m in multiplier]

        # 我們只Pruning第三層以後的Layer
        for i in range(3, 7):
            bandwidth[i] = int(bandwidth[i] * width_mult)
            groups_num[i] = int(groups_num[i] * width_mult)
        ...
        self.cnn = nn.Sequential(
            ...
            nn.Sequential(
                ...
                nn.Conv2d(bandwidth[0], bandwidth[1], 3, 1, 1, groups=groups_num[0]),
                # Batch Normalization
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[1], bandwidth[2], 3, 1, 1, groups=groups_num[1]),
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[2], bandwidth[3], 3, 1, 1, groups=groups_num[2]),
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[3], bandwidth[4], 3, 1, 1, groups=groups_num[3]),
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[4], bandwidth[5], 3, 1, 1, groups=groups_num[4]),
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[5], bandwidth[6], 3, 1, 1, groups=groups_num[5]),
            ...
            nn.Sequential(
                nn.Conv2d(bandwidth[6], bandwidth[7], 3, 1, 1, groups=groups_num[6]),
...
for epoch in range(num_epoch):
    ...
    with torch.no_grad():
        ...
        if val_acc > now_best_acc:
            ...
            torch.save(model.state_dict(), 'Architecture_design.bin')
        #將結果 print 出來
...
student_net.load_state_dict(torch.load('Architecture_design.bin'))
